    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-border flex-shrink-0">
          ...
          <button onClick={() => setIsOpen(false)} className="w-9 h-9 flex items-center justify-center rounded-lg hover:bg-muted transition-colors">
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {items.length === 0 ? (
            <div className="text-center py-16">
              ...
              <button onClick={() => setIsOpen(false)} className="mt-4 text-primary text-sm hover:underline">
                Continuar comprando
              </button>
            </div>
          ) : (
              ...
              <div key={item.product.id} className="flex gap-4 p-3 bg-card rounded-xl border border-border">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    <button onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="w-7 h-7 flex items-center justify-center rounded-md border border-border hover:bg-muted transition-colors">
                      <Minus size={12} />
                    </button>
                    ...
                    <button onClick={() => removeItem(item.product.id)} className="ml-auto w-7 h-7 flex items-center justify-center rounded-md text-destructive/60 hover:text-destructive hover:bg-destructive/10 transition-colors">
                      <Trash2 size={12} />
                    </button>
                  </div>
                </div>
        ...
        {items.length > 0 && (
          <div className="p-5 border-t border-border bg-background flex-shrink-0">
            ...
            <button onClick={clearCart} className="w-full mt-2 text-xs text-muted-foreground hover:text-destructive transition-colors py-2">
              Limpar sacola
            </button>
          </div>
        )}
